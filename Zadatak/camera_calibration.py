...
for path in glob.glob('camera_cal/*.jpg'):
    ...
    ret, corners = cv2.findChessboardCorners(preprocessed_image, (rows, cols), None)

    # Make sure the chess board pattern was found in the image
    if ret:
        # Refine the corner position
        corners = cv2.cornerSubPix(preprocessed_image, corners, (11, 11), (-1, -1), criteria)
...
ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objectPointsArray, imgPointsArray, preprocessed_image.shape[::-1], None, None)
